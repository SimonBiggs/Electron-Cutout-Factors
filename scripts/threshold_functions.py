...
def single_fit_give(xTest,yTest,xData,yData,zData):
    ...
    initialFitReturn = SmoothBivariateSpline(xData,yData,zData).ev(xTest,yTest)
    ...
    posFitReturn = SmoothBivariateSpline(adjXData,adjYData,posAdjZData).ev(xTest,yTest)
    negFitReturn = SmoothBivariateSpline(adjXData,adjYData,negAdjZData).ev(xTest,yTest)
...
def fit_give(xTest,yTest,xData,yData,zData):
    ...
    if np.size(dim) == 0:
        give = single_fit_give(xTest,yTest,xData,yData,zData)
    ...
    if np.size(dim) == 1:
        for i in range(dim[0]):
            give[i] = single_fit_give(xTest[i],yTest[i],xData,yData,zData)
    ...
    for i in range(dim[0]):
        for j in range (dim[1]):
            give[i,j] = single_fit_give(xTest[i,j],yTest[i,j],xData,yData,zData)
